...
struct swe_departure_to_target {
	...
	Kokkos::View<double**, Kokkos::LayoutRight> arrival_div;
	Kokkos::View<double**, Kokkos::LayoutRight> arrival_h;
	Kokkos::View<double**, Kokkos::LayoutRight> new_vors;
	Kokkos::View<double**, Kokkos::LayoutRight> new_divs;
	Kokkos::View<double**, Kokkos::LayoutRight> new_h;
	...
	swe_departure_to_target(Kokkos::View<double**, Kokkos::LayoutRight>& dep_x_, Kokkos::View<double**, Kokkos::LayoutRight>& dep_y_, Kokkos::View<double**, Kokkos::LayoutRight>& dep_z_, 
						Kokkos::View<double**, Kokkos::LayoutRight>& arrival_vor_, Kokkos::View<double**, Kokkos::LayoutRight>& arrival_div_, Kokkos::View<double**, Kokkos::LayoutRight>& arrival_h_, 
						Kokkos::View<double**, Kokkos::LayoutRight>& new_vors_, Kokkos::View<double**, Kokkos::LayoutRight>& new_divs_, Kokkos::View<double**, Kokkos::LayoutRight>& new_h_,
						...
						dep_x(dep_x_), dep_y(dep_y_), dep_z(dep_z_), arrival_vor(arrival_vor_), arrival_div(arrival_div_), arrival_h(arrival_h_), new_vors(new_vors_), new_divs(new_divs_), new_h(new_h_), 
		...
		for (int j = 0; j < (degree+1)*(degree+1); j++) { // loop over points in each panel
			...
			for (int m = 0; m < degree+1; m++) { // xi loop
				for (int n = 0; n < degree+1; n++) { // eta loop
					...
					new_divs(i,j) += arrival_div(panel_index - offset, one_d_index) * bli_basis_vals[one_d_index];
					new_h(i,j) += arrival_h(panel_index - offset, one_d_index) * bli_basis_vals[one_d_index];
					for (int k = 0; k < passive_tracers.extent_int(2); k++) {
...
struct swe_tendency_computation {
	...
	Kokkos::View<double**, Kokkos::LayoutRight> height_lap;
	Kokkos::View<CubedSpherePanel*> cubed_sphere_panels;
	double omega;
	int degree;
	int offset;

	swe_tendency_computation(Kokkos::View<double**, Kokkos::LayoutRight>& xcos_, Kokkos::View<double**, Kokkos::LayoutRight>& ycos_, Kokkos::View<double**, Kokkos::LayoutRight>& zcos_, 
				...
				Kokkos::View<double**, Kokkos::LayoutRight>& div_tend_, Kokkos::View<double**, Kokkos::LayoutRight>& height_,  Kokkos::View<double**, Kokkos::LayoutRight>& height_tend_, Kokkos::View<double**, Kokkos::LayoutRight>& height_lap_, 
				...
				vor(vor_), vor_tend(vor_tend_), div(div_), div_tend(div_tend_), height(height_), height_tend(height_tend_), height_lap(height_lap_), cubed_sphere_panels(cubed_sphere_panels_), omega(omega_), degree(degree_), offset(offset_) {}
	...
	void operator()(const int i) const {
		...
		double vel_grad_11[121], vel_grad_12[121], vel_grad_13[121], vel_grad_21[121], vel_grad_22[121], vel_grad_23[121], vel_grad_31[121], vel_grad_32[121], vel_grad_33[121];
		single_panel_grad(vel_grad_11, vel_grad_12, vel_grad_13, &vel_x(i,0), degree, min_xi, max_xi, min_eta, max_eta, cubed_sphere_panels(i).face);
		single_panel_grad(vel_grad_21, vel_grad_22, vel_grad_23, &vel_y(i,0), degree, min_xi, max_xi, min_eta, max_eta, cubed_sphere_panels(i).face);
		single_panel_grad(vel_grad_31, vel_grad_32, vel_grad_33, &vel_z(i,0), degree, min_xi, max_xi, min_eta, max_eta, cubed_sphere_panels(i).face);
		double abs_vor, abs_vor_tend, x, y, z, grad_comp, vel_u, dp;
		double lat, lon;
		for (int j = 0; j < xcos.extent_int(1); j++) {
			...
			// div_tend(i,j) -= grad_comp / (6371000.0*6371000.0);
...
struct swe_apply_tendencies {
	...
	swe_apply_tendencies(Kokkos::View<double**, Kokkos::LayoutRight>& vor_, Kokkos::View<double**, Kokkos::LayoutRight>& vor_tend_, Kokkos::View<double**, Kokkos::LayoutRight>& new_vor_, 
...
struct swe_disp_update {
	Kokkos::View<double**, Kokkos::LayoutRight> vel;
	Kokkos::View<double**, Kokkos::LayoutRight> disp;
	double dt;
	int offset;

	swe_disp_update(Kokkos::View<double**, Kokkos::LayoutRight>& vel_, Kokkos::View<double**, Kokkos::LayoutRight>& disp_, double dt_, int offset_) : vel(vel_), disp(disp_), dt(dt_), offset(offset_) {}
...
struct swe_disp_interp {
	...
	swe_disp_interp(Kokkos::View<double**, Kokkos::LayoutRight>& xcos_, Kokkos::View<double**, Kokkos::LayoutRight>& ycos_, Kokkos::View<double**, Kokkos::LayoutRight>& zcos_, 
				Kokkos::View<double**, Kokkos::LayoutRight>& vel_x_, Kokkos::View<double**, Kokkos::LayoutRight>& vel_y_, Kokkos::View<double**, Kokkos::LayoutRight>& vel_z_, 
...
void swe_displacement_upward_pass(const RunConfig& run_config, Kokkos::View<double**, Kokkos::LayoutRight>& xcos, Kokkos::View<double**, Kokkos::LayoutRight>& ycos, 
	...
	Kokkos::parallel_for(run_config.active_panel_count, swe_disp_update(vel_x, disp_x, dt, offset));
	Kokkos::parallel_for(run_config.active_panel_count, swe_disp_update(vel_y, disp_y, dt, offset));
	Kokkos::parallel_for(run_config.active_panel_count, swe_disp_update(vel_z, disp_z, dt, offset));
	Kokkos::parallel_for(offset, swe_disp_interp(xcos, ycos, zcos, vel_x, vel_y, vel_z, disp_x, disp_y, disp_z, interp_vals, cubed_sphere_panels, dt, run_config.interp_degree));
}

struct swe_compute_arrival_values {
	Kokkos::View<double**, Kokkos::LayoutRight> xcos;
	Kokkos::View<double**, Kokkos::LayoutRight> ycos;
	Kokkos::View<double**, Kokkos::LayoutRight> zcos;
	Kokkos::View<double**, Kokkos::LayoutRight> vor;
	...
	swe_compute_arrival_values(Kokkos::View<double**, Kokkos::LayoutRight>& xcos_, Kokkos::View<double**, Kokkos::LayoutRight>& ycos_, Kokkos::View<double**, Kokkos::LayoutRight>& zcos_,
	...
	void operator()(const int i) const {
		...
		double vor1, vor2, vor3, vor4, div1, div2, div3, div4, h1, h2, h3, h4, x, y, z, xieta[2], bli_basis_vals[121];
		int panel_index, one_d_index;
		for (int j = 0; j < jmax; j++) {
			// k1
			vor1 = vor_tend_1(i,j);
			div1 = div_tend_1(i,j);
			h1 = height_tend_1(i,j);
			...
			for (int m = 0; m < degree+1; m++) {
				for (int n = 0; n < degree+1; n++) {
					...
					div2 += div_tend_2(panel_index - offset, one_d_index) * bli_basis_vals[one_d_index];
					h2 += height_tend_2(panel_index - offset, one_d_index) * bli_basis_vals[one_d_index];
					one_d_index += 1;
				}
			}
			...
			for (int m = 0; m < degree+1; m++) {
				for (int n = 0; n < degree+1; n++) {
					...
					div3 += div_tend_3(panel_index - offset, one_d_index) * bli_basis_vals[one_d_index];
					h3 += height_tend_3(panel_index - offset, one_d_index) * bli_basis_vals[one_d_index];
					one_d_index += 1;
				}
			}
			...
			for (int m = 0; m < degree+1; m++) {
				for (int n = 0; n < degree+1; n++) {
					...
					div3 += div_tend_4(panel_index - offset, one_d_index) * bli_basis_vals[one_d_index];
					h3 += height_tend_4(panel_index - offset, one_d_index) * bli_basis_vals[one_d_index];
					one_d_index += 1;
				}
			}
			...
			// a_div(i,j) = div(i,j) + dt/6.0*(div1 + 2.0*div2 + 2.0*div3 + div4);
			// a_height(i,j) = height(i,j) + dt/6.0*(h1 + 2.0*h2 + 2.0*h3 + h4);
...
struct swe_find_departure_points {
	Kokkos::View<double**, Kokkos::LayoutRight> xcos;
	Kokkos::View<double**, Kokkos::LayoutRight> ycos;
	Kokkos::View<double**, Kokkos::LayoutRight> zcos;
	...
	swe_find_departure_points(Kokkos::View<double**, Kokkos::LayoutRight>& xcos_, Kokkos::View<double**, Kokkos::LayoutRight>& ycos_, Kokkos::View<double**, Kokkos::LayoutRight>& zcos_, 
...
struct swe_height_correction {
	...
	swe_height_correction(Kokkos::View<double**, Kokkos::LayoutRight>& xcos_, Kokkos::View<double**, Kokkos::LayoutRight>& ycos_, Kokkos::View<double**, Kokkos::LayoutRight>& zcos_, 
							Kokkos::View<double**, Kokkos::LayoutRight>& vel_x_, Kokkos::View<double**, Kokkos::LayoutRight>& vel_y_, Kokkos::View<double**, Kokkos::LayoutRight>& vel_z_, 
							Kokkos::View<double**, Kokkos::LayoutRight>& height_, Kokkos::View<double**, Kokkos::LayoutRight>& height_tend_, Kokkos::View<double**, Kokkos::LayoutRight>& height_lap_, 
							Kokkos::View<CubedSpherePanel*>& cubed_sphere_panels_, int offset_, double dt_, int degree_) : xcos(xcos_), ycos(ycos_), zcos(zcos_), vel_x(vel_x_), vel_y(vel_y_), vel_z(vel_z_), 
							height(height_), height_tend(height_tend_), height_lap(height_lap_), cubed_sphere_panels(cubed_sphere_panels_), offset(offset_), dt(dt_), degree(degree_) {}

	KOKKOS_INLINE_FUNCTION
	void operator()(const int i) const {
		int jmax = xcos.extent_int(1);
		double x, y, z, xieta[2], bli_basis_vals[121], panel_index, new_h;
		int one_d_index;
		for (int j = 0; j < jmax; j++) {
			x = xcos(i,j) + dt * vel_x(i,j);
			y = ycos(i,j) + dt * vel_y(i,j);
			z = zcos(i,j) + dt * vel_z(i,j);
			project_to_sphere(x, y, z);
			xieta_from_xyz(x, y, z, xieta);
			panel_index = point_locate_panel(cubed_sphere_panels, x, y, z);
			interp_vals_bli(bli_basis_vals, xieta[0], xieta[1], Kokkos::numbers::pi/4.0*cubed_sphere_panels(panel_index).min_xi, Kokkos::numbers::pi/4.0*cubed_sphere_panels(panel_index).max_xi, 
			...
			for (int m = 0; m < degree+1; m++) {
				for (int n = 0; n < degree+1; n++) {
					new_h += (height(panel_index-offset,one_d_index) + dt*height_tend(panel_index-offset,one_d_index))*bli_basis_vals[one_d_index];
					one_d_index += 1;
				}
			}
			height_lap(i,j) -= 8.0*new_h/0.1;
		}
	}
};

void swe_back_rk4_step(const RunConfig& run_config, Kokkos::View<double**, Kokkos::LayoutRight>& xcos, Kokkos::View<double**, Kokkos::LayoutRight>& ycos, 
	...
	laplacian(run_config, height_lap_1, effective_height, cubed_sphere_panels);
	...
	swe_vel_interactions(run_config, disp_x, disp_y, disp_z, proxy_target_1, proxy_target_2, proxy_target_3, proxy_source_vors, proxy_source_divs, interaction_list, cubed_sphere_panels, interp_vals);
	...
	Kokkos::parallel_for(run_config.active_panel_count, swe_tendency_computation(xcos, ycos, zcos, vel_x, vel_y, vel_z, vors, vor_update_1, divs, div_update_1, height, height_update_1, height_lap_1, cubed_sphere_panels, run_config.omega,run_config.interp_degree, offset));
	...
	swe_displacement_upward_pass(run_config, xcos, ycos, zcos, vel_x, vel_y, vel_z, disp_x, disp_y, disp_z, cubed_sphere_panels, interp_vals, 0.5*run_config.delta_t);
	Kokkos::parallel_for(run_config.active_panel_count, swe_apply_tendencies(vors, vor_update_1, disp_vors, divs, div_update_1, disp_divs, height, height_update_1, disp_heights, area, disp_areas, 0.5*run_config.delta_t));
	...
	upward_pass(run_config, interp_vals, cubed_sphere_panels, disp_areas, disp_divs, proxy_source_divs);
	upward_pass(run_config, interp_vals, cubed_sphere_panels, disp_areas, effective_height, proxy_source_heights);
	swe_vel_interactions(run_config, disp_x, disp_y, disp_z, proxy_target_1, proxy_target_2, proxy_target_3, proxy_source_vors, proxy_source_divs, interaction_list, cubed_sphere_panels, interp_vals);
	...
	Kokkos::parallel_for(run_config.active_panel_count, swe_height_correction(xcos, ycos, zcos, vel_x, vel_y, vel_z, height, height_update_1, height_lap_2, cubed_sphere_panels, offset, 0.5*run_config.delta_t, run_config.interp_degree));
	...
	Kokkos::parallel_for(run_config.active_panel_count, swe_tendency_computation(xcos, ycos, zcos, vel_x_2, vel_y_2, vel_z_2, vors, vor_update_2, divs, div_update_2, height, height_update_2, height_lap_2, cubed_sphere_panels, run_config.omega,run_config.interp_degree, offset));
	...
	swe_displacement_upward_pass(run_config, xcos, ycos, zcos, vel_x_2, vel_y_2, vel_z_2, disp_x, disp_y, disp_z, cubed_sphere_panels, interp_vals, 0.5*run_config.delta_t);
	Kokkos::parallel_for(run_config.active_panel_count, swe_apply_tendencies(vors, vor_update_2, disp_vors, divs, div_update_2, disp_divs, height, height_update_2, disp_heights, area, disp_areas, 0.5*run_config.delta_t));
	...
	Kokkos::parallel_for(run_config.active_panel_count, swe_tendency_computation(xcos, ycos, zcos, vel_x_3, vel_y_3, vel_z_3, vors, vor_update_3, divs, div_update_3, height, height_update_3, height_lap_3, cubed_sphere_panels, run_config.omega,run_config.interp_degree, offset));
	...
	swe_displacement_upward_pass(run_config, xcos, ycos, zcos, vel_x_3, vel_y_3, vel_z_3, disp_x, disp_y, disp_z, cubed_sphere_panels, interp_vals, run_config.delta_t);
	Kokkos::parallel_for(run_config.active_panel_count, swe_apply_tendencies(vors, vor_update_3, disp_vors, divs, div_update_3, disp_divs, height, height_update_3, disp_heights, area, disp_areas, 0.5*run_config.delta_t));
	...
	Kokkos::parallel_for(run_config.active_panel_count, swe_tendency_computation(xcos, ycos, zcos, vel_x_4, vel_y_4, vel_z_4, vors, vor_update_4, divs, div_update_4, height, height_update_4, height_lap_4, cubed_sphere_panels, run_config.omega,run_config.interp_degree, offset));
	...
	Kokkos::View<double**, Kokkos::LayoutRight> arrival_div ("arrival div", run_config.active_panel_count, dim2size);
	Kokkos::View<double**, Kokkos::LayoutRight> arrival_h ("arrival height", run_config.active_panel_count, dim2size);
	Kokkos::parallel_for(run_config.active_panel_count, swe_compute_arrival_values(xcos, ycos, zcos, vors, divs, height, arrival_vor, arrival_div, arrival_h, vel_x, vel_y, vel_z, vel_x_2, vel_y_2, vel_z_2, vel_x_3, vel_y_3, vel_z_3, 
	...
	Kokkos::parallel_for(xcos.extent_int(0), swe_find_departure_points(xcos, ycos, zcos, vel_x, vel_y, vel_z, vel_x_2, vel_y_2, vel_z_2, vel_x_3, vel_y_3, vel_z_3, vel_x_4, vel_y_4, vel_z_4, dep_x, dep_y, dep_z, cubed_sphere_panels, run_config.delta_t, offset, run_config.interp_degree));
	...
	Kokkos::parallel_for(run_config.active_panel_count, swe_departure_to_target(dep_x, dep_y, dep_z, arrival_vor, arrival_div, arrival_h, new_vors, new_divs, new_h, passive_tracers, new_passive_tracers, cubed_sphere_panels, run_config.interp_degree, offset, run_config.omega));
