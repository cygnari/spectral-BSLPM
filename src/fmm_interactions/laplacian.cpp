...
void laplacian_interactions(const RunConfig& run_config, Kokkos::View<double**, Kokkos::LayoutRight>& disp_x, Kokkos::View<double**, Kokkos::LayoutRight>& disp_y, 
    ...
    Kokkos::parallel_for(Kokkos::RangePolicy(lb, ub), lap_panel_interaction(disp_x, disp_y, disp_z, target_pots, source_vals, interp_vals, cubed_sphere_panels, interaction_list, offset, run_config.kernel_eps));
    Kokkos::fence();
}
