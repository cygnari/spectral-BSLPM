...
struct bve_ssw_forcing {
	...
	void operator()(const int i) const {
		...
		double lat, lon, acomp, bcomp = 0, ccomp;
...
struct bve_ssw_forcing_2 {
	...
	bve_ssw_forcing_2(Kokkos::View<double**, Kokkos::LayoutRight>& xcos_, Kokkos::View<double**, Kokkos::LayoutRight>& ycos_, Kokkos::View<double**, Kokkos::LayoutRight>& zcos_,  
					Kokkos::View<double**, Kokkos::LayoutRight>& vorticity_, Kokkos::View<double**, Kokkos::LayoutRight>& effective_vorticity_, double time_, double omega_) : xcos(xcos_), 
					ycos(ycos_), zcos(zcos_), vorticity(vorticity_), effective_vorticity(effective_vorticity_), time(time_), omega(omega_) {}
	...
	void operator()(const int i) const {
		...
		for (int j = 0; j < jmax; j++) {
			...
			effective_vorticity(i,j) = vorticity(i,j) + 1.2 * omega * acomp * bcomp * Kokkos::cos(2.0*lon);
		}
	}
};
...
void bve_forcing(const RunConfig& run_config, Kokkos::View<double**, Kokkos::LayoutRight>& xcos, 
	...
	if (run_config.forcing) {
		...
		} else if (run_config.forcing_type == "ssw2") {
			Kokkos::parallel_for(run_config.active_panel_count, bve_ssw_forcing_2(xcos, ycos, zcos, vorticity, effective_vorticity, time, run_config.omega));
		}
	} else {
