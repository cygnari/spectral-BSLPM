...
struct inv_lap {
	...
	void operator()(const int i, const int j) const {
		if (i != j) {
			...
			Kokkos::atomic_add(&soln(i), -0.07957747154594767 * log(1-dp) * area(j) * pots(j));
		}
	}
};
...
void direct_sum_inv_lap(const RunConfig& run_config, Kokkos::View<double*> xcos, Kokkos::View<double*> ycos, Kokkos::View<double*> zcos, Kokkos::View<double*> area, Kokkos::View<double*> pots, Kokkos::View<double*> soln) {
	Kokkos::parallel_for(Kokkos::MDRangePolicy({run_config.oned_lb, 0},{run_config.oned_ub, run_config.point_count}), inv_lap(xcos, ycos, zcos, area, pots, soln));
	Kokkos::fence();
