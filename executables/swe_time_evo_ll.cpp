...
int main(int argc, char* argv[]) {
	...
	Kokkos::initialize(argc, argv); {
		...
		for (int i = 0; i < lat_count; i++) {
			lats(i) = -90.0 + run_config.grid_spacing * i;
		}
		for (int i = 0; i < lon_count; i++) {
			lons(i) = run_config.grid_spacing * i;
		}
		...
		Kokkos::View<CubedSpherePanel*, Kokkos::HostSpace> cubed_sphere_panels ("cubed sphere panels", 0);
		Kokkos::View<int**, Kokkos::LayoutRight, Kokkos::HostSpace> leaf_panel_points ("leaf panel points", 0, 0);

		initialize_cube_sphere_irreg_points(run_config, xcos, ycos, zcos, cubed_sphere_panels, leaf_panel_points);
		...
		if (run_config.mpi_id == 0) {
			...
			std::cout << "cubed sphere panels: " << run_config.panel_count << std::endl;
		...
		int lat_varid, lon_varid, time_varid, uvel_id, vvel_id, vor_id, div_id, height_id, topo_id, x_id, y_id, z_id;
		...
		dual_tree_traversal_irreg(run_config, cubed_sphere_panels, interaction_list);
		Kokkos::fence();
		MPI_Barrier(MPI_COMM_WORLD);
		end = std::chrono::steady_clock::now();
		if (run_config.mpi_id == 0) {
		...
		for (int t = 0; t < run_config.time_steps; t++) {
			...
			swe_back_rk4_step_ll(run_config, d_xcos, d_ycos, d_zcos, d_area, d_vors, d_divs, d_height, d_topo, d_cubed_sphere_panels, d_interaction_list, d_leaf_panel_points, d_vel_x, d_vel_y, d_vel_z, run_config.delta_t*t);
